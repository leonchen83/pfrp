...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      fs match {
        case Nil => Future(Nil)
        case head :: tail => head.flatMap(x => all(tail).flatMap(xx => Future(x :: xx)))
      }
    }
     ...
     * The delay may block the execution thread of its future until the specified time period elapses,
     * but it should not block the caller thread.
     */
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      cancel.p.tryCompleteWith(f(cancel.cancellationToken))
      cancel
    }
  }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]
      f onComplete {
        case e=>p.tryComplete(Try(cont(f)))
      }
      p.future
//      Future {
//        val s = Await.ready(f, Duration.Inf)
//        cont(s)
//      }
    }
