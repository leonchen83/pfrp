...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = {
      searchTerms.concatRecovered(e => Observable.from(Search.wikipediaSuggestion(e)))
    }
    ...
    val selections: Observable[String] = {
      val s = button.clicks.filter(e=>suggestionList.selection.items.size!=0)
      s.map(e => suggestionList.selection.items.head)
    }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = {
      selections.concatRecovered(e => Observable.from(Search.wikipediaPage(e)))
    }
