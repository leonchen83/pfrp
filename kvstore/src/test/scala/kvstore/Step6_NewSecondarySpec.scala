...
class Step6_NewSecondarySpec extends TestKit(ActorSystem("Step6NewSecondarySpec"))
  ...
  test("case3: Primary must stop replication to removed replicas and waive their outstanding acknowledgements") {
    val arbiter = TestProbe()
        ...
        val user = session(primary)
    val secondary = TestProbe()
    val secondary1 = TestProbe()

    arbiter.expectMsg(Join)
    arbiter.send(primary, JoinedPrimary)
    arbiter.send(primary, Replicas(Set(primary, secondary.ref,secondary1.ref)))

    val ack1 = user.set("k1", "v1")
    secondary.expectMsg(Snapshot("k1", Some("v1"), 0L))
    secondary.reply(SnapshotAck("k1", 0L))

    secondary1.expectMsg(Snapshot("k1", Some("v1"), 0L))
    secondary1.reply(SnapshotAck("k1", 0L))
    user.waitAck(ack1)

    val ack2 = user.set("k1", "v2")
    secondary.expectMsg(Snapshot("k1", Some("v2"), 1L))
    secondary.reply(SnapshotAck("k1", 1L))
    secondary1.expectMsg(Snapshot("k1", Some("v2"), 1L))
    arbiter.send(primary, Replicas(Set(primary,secondary.ref)))
    user.waitAck(ack2)
  }
  ...
  test("case5: Primary and secondaries must work in concert when persistence and communication to secondaries is unreliable") {

    val arbiter = TestProbe()

    val (primary, user) = createPrimary(arbiter, "case5-primary", flakyPersistence = true)

    user.setAcked("k1", "v1")

    val (secondary1, replica1) = createSecondary(arbiter, "case5-secondary1", flakyForwarder = true, flakyPersistence = true)
    val (secondary2, replica2) = createSecondary(arbiter, "case5-secondary2", flakyForwarder = true, flakyPersistence = true)
    ...
    val (secondary3, replica3) = createSecondary(arbiter, "case5-secondary3", flakyForwarder = true, flakyPersistence = true)
